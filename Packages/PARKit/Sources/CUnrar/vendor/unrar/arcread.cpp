...
size_t Archive::ReadHeader15()
{
  ...
  // LOCAL PATCH (ModernPAR, see ../VENDORED.txt): records whether a file/service
  // header passed its case-branch CRC check (CRCProcessedOnly semantics), so the
  // generic re-check below can be skipped ONLY for headers that verified clean.
  bool FileHeaderCRCValid=true;
    ...
    case HEAD_SERVICE:
      {
        ...
        FileHeaderCRCValid=hd->HeadCRC==HeaderCRC;  // LOCAL PATCH (ModernPAR)
  ...
  // approach here.
  //
  // LOCAL PATCH (ModernPAR, see ../VENDORED.txt): also skip this generic
  // full-buffer re-check for (a) file/service headers WHOSE CASE-BRANCH CRC
  // CHECK PASSED — they were already verified above with the correct
  // CRCProcessedOnly semantics (an old-style embedded file comment makes the
  // stored CRC cover only the processed part, so the full-buffer CRC here
  // mismatches on perfectly good headers); genuinely corrupt file headers
  // (FileHeaderCRCValid == false) still take this block, preserving
  // upstream's RARX_CRC classification and the FailedHeaderDecryption stop
  // for encrypted headers — and (b) a main header with an embedded RAR<=2.9
  // comment, whose comment bytes are deliberately not read into Raw (see the
  // MHD_COMMENT branch above), so its stored CRC cannot be verified from
  // this buffer. Without this, every commented RAR 1.5-2.x archive silently
  // set the sticky RARX_CRC error code at open, which unrar.dll's call tails
  // then misreport as ERAR_BAD_DATA on every subsequent successful call.
  if (ShortBlock.HeadCRC!=HeaderCRC && ShortBlock.HeaderType!=HEAD3_SIGN &&
      ...
      ((ShortBlock.HeaderType!=HEAD_FILE && ShortBlock.HeaderType!=HEAD_SERVICE) ||
       !FileHeaderCRCValid) &&
      (ShortBlock.HeaderType!=HEAD_MAIN || (ShortBlock.Flags & MHD_COMMENT)==0))
  {
