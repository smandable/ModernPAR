...
int PASCAL RARReadHeaderEx(HANDLE hArcData,struct RARHeaderDataEx *D)
{
  ...
  try
  {
    // LOCAL PATCH (ModernPAR, see ../VENDORED.txt): reset the per-call error
    // exactly as ProcessFile does. Without this, a per-file error from a
    // previous RARProcessFile call (e.g. one corrupt file mid-archive) stays
    // in Cmd->DllError and this function's success tail re-reports it for
    // every following header, aborting callers' keep-extracting loops.
    Data->Cmd.DllError=0;
    if ((Data->HeaderSize=(int)Data->Arc.SearchBlock(HEAD_FILE))<=0)
    {
      ...
      // LOCAL PATCH (ModernPAR, vendor/VENDORED.txt): BrokenHeader is a STICKY
      // warning flag. The partial-CRC quirk on RAR<=2.9 main headers with
      // embedded comments (arcread.cpp reads only SIZEOF_MAINHEAD3 of the
      // commented main header, then GetCRC15 over the partial buffer
      // mismatches) sets it during the open, which made this path misreport a
      // clean end-of-archive as ERAR_BAD_DATA for every commented RAR 1.5-2.x
      // archive. The CLI treats BrokenHeader as a warning and keeps going;
      // report BAD_DATA only when the scan did not stop exactly at the
      // archive end (a clean end always lands on FileLength(); a corrupt
      // header whose garbage size fields jump PAST the end must still be
      // damage, hence `!=` and not `<`).
      if (Data->Arc.BrokenHeader && Data->Arc.CurBlockPos!=Data->Arc.FileLength())
